"use client";

import { t } from "i18next";
import { OurAppImage, OurAppPattern, OurAppPattern2 } from "@assets";
import { useIsRTL } from "@hooks";

const OurApp = () => {
  const isRTL = useIsRTL();

  return (
    <section className="py-16 bg-white relative overflow-hidden ourApp-mobile">
      {/* Animated Pattern Image */}
      <div
        className={`mobile-animation absolute  transform ${
          isRTL ? "left-0" : "right-0"
        } z-0`}
      >
        <img
          src={OurAppPattern}
          alt="App Pattern"
          className="w-64 h-auto animate-float"
        />
      </div>

      <div className="container mx-auto px-4 relative z-10">
        <div className="grid lg:grid-cols-1 gap-12 items-center">
          <div className="text-center lg:text-center py-8">
            <span className="text-[#fd671a] text-md font-semibold">
              {t("our-app.subtitle")}
            </span>
            <h2 className="font-size-mobile-heading text-4xl lg:text-4xl font-bold text-gray-900 leading-tight mt-2">
              {t("our-app.title")}
            </h2>
          </div>
        </div>
        <div className="grid lg:grid-cols-2 gap-12 items-center  mt-10 mt-mobile-24">
          {/* Left Column - Text Content */}
          <div className="relative">
            <img
              src={OurAppImage}
              alt="Our App Interface"
              className="w-full h-auto rounded-lg "
            />
          </div>

          {/* Right Column - App Interface */}
          <div className="space-y-8">
            {/* Header */}
            <div className="text-center lg:text-start">
              <h2 className="font-size-mobile-heading text-4xl lg:text-4xl font-bold text-gray-900 leading-tight mt-2">
                {t("our-app.collaborate-title")}
              </h2>
            </div>

            {/* Description */}
            <p className="text-gray-600 text-lg leading-relaxed">
              {t("our-app.description")}
            </p>

            {/* Features List */}
            <div className="space-y-4">
              <div
                className={`flex items-center ${
                  isRTL ? "space-x-reverse" : ""
                } space-x-3`}
              >
                <div
                  className="w-6 h-6 rounded-full flex items-center justify-center"
                  style={{ backgroundColor: "#A4DB74" }}
                >
                  <svg
                    className="w-4 h-4 text-white"
                    fill="currentColor"
                    viewBox="0 0 20 20"
                  >
                    <path
                      fillRule="evenodd"
                      d="M16.707 5.293a1 1 0 010 1.414l-8 8a1 1 0 01-1.414 0l-4-4a1 1 0 011.414-1.414L8 12.586l7.293-7.293a1 1 0 011.414 0z"
                      clipRule="evenodd"
                    />
                  </svg>
                </div>
                <span
                  className={`font-semibold ${
                    isRTL ? "text-right" : "text-left"
                  }`}
                >
                  {t("our-app.feature1")}
                </span>
              </div>
              <div
                className={`flex items-center ${
                  isRTL ? "space-x-reverse" : ""
                } space-x-3`}
              >
                <div
                  className="w-6 h-6 rounded-full flex items-center justify-center"
                  style={{ backgroundColor: "#A4DB74" }}
                >
                  <svg
                    className="w-4 h-4 text-white"
                    fill="currentColor"
                    viewBox="0 0 20 20"
                  >
                    <path
                      fillRule="evenodd"
                      d="M16.707 5.293a1 1 0 010 1.414l-8 8a1 1 0 01-1.414 0l-4-4a1 1 0 011.414-1.414L8 12.586l7.293-7.293a1 1 0 011.414 0z"
                      clipRule="evenodd"
                    />
                  </svg>
                </div>
                <span
                  className={`font-semibold ${
                    isRTL ? "text-right" : "text-left"
                  }`}
                >
                  {t("our-app.feature2")}
                </span>
              </div>
            </div>

            {/* App Download Buttons */}
            <div className="flex flex-row gap-4">
              <button className="flex items-center justify-center bg-black text-white px-6 py-3 rounded-lg hover:bg-gray-800 transition-colors">
                <svg
                  className="w-8 h-8 mr-3"
                  viewBox="0 0 24 24"
                  fill="currentColor"
                >
                  <path d="M18.71 19.5c-.83 1.24-1.71 2.45-3.05 2.47-1.34.03-1.77-.79-3.29-.79-1.53 0-2 .77-3.27.82-1.31.05-2.3-1.32-3.14-2.53C4.25 17 2.94 12.45 4.7 9.39c.87-1.52 2.43-2.48 4.12-2.51 1.28-.02 2.5.87 3.29.87.78 0 2.26-1.07 3.81-.91.65.03 2.47.26 3.64 1.98-.09.06-2.17 1.28-2.15 3.81.03 3.02 2.65 4.03 2.68 4.04-.03.07-.42 1.44-1.38 2.83M13 3.5c.73-.83 1.94-1.46 2.94-1.5.13 1.17-.34 2.35-1.04 3.19-.69.85-1.83 1.51-2.95 1.42-.15-1.15.41-2.35 1.05-3.11z" />
                </svg>
                <div className="text-left">
                  <div className="text-xs">Download on the</div>
                  <div className="text-sm font-semibold">App Store</div>
                </div>
              </button>
              <button className="flex items-center justify-center bg-black text-white px-6 py-3 rounded-lg hover:bg-gray-800 transition-colors">
                <svg
                  className="w-8 h-8 mr-3"
                  viewBox="0 0 24 24"
                  fill="currentColor"
                >
                  <path d="M3,20.5V3.5C3,2.91 3.34,2.39 3.84,2.15L13.69,12L3.84,21.85C3.34,21.6 3,21.09 3,20.5M16.81,15.12L6.05,21.34L14.54,12.85L16.81,15.12M20.16,10.81C20.5,11.08 20.75,11.5 20.75,12C20.75,12.5 20.53,12.9 20.18,13.18L17.89,14.5L15.39,12L17.89,9.5L20.16,10.81M6.05,2.66L16.81,8.88L14.54,11.15L6.05,2.66Z" />
                </svg>
                <div className="text-left">
                  <div className="text-xs">GET IT ON</div>
                  <div className="text-sm font-semibold">Google Play</div>
                </div>
              </button>
            </div>
          </div>
        </div>
      </div>

      <div
        className={`absolute bottom-0  transform ${
          isRTL ? "right-0" : "left-0"
        } z-0`}
      >
        <img
          src={OurAppPattern2}
          alt="App Pattern"
          className="w-3/5 h-auto animate-float"
        />
      </div>
    </section>
  );
};

export default OurApp;
