...
const PortfolioDetailsPage = () => {
  ...
  // Get project data from API using the id as projectId
  const {
    data: projectResponse,
    isSuccess: isProjectSuccess,
    isLoading: isProjectLoading,
  } = useGetQuery({
    endpoint: API_ENDPOINTS.getProject(id || ""),
  });
      ...
      <section className="container mx-auto md:p-10 p-6">
        <div className="relative w-full max-w-4xl mx-auto">
          ...
          <div className="flex items-center gap-2 text-gray-600 mb-6 text-sm">
            <FiMapPin />
            <span>{isRTL ? project.city.arName : project.city.enName}</span>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-10">
            {[
              ...
              <div
                key={card.title}
                className="rounded-md bg-[#F5F5F5] px-6 py-4 flex flex-col items-start gap-4 shadow-sm hover:shadow transition-all"
              >
